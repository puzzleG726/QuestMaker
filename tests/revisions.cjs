...
(async()=>{
 ...
 const num=async(s,v)=>{if(await page.locator(s).getAttribute('type')==='range')await page.locator(s).evaluate((el,v)=>{el.value=v;el.dispatchEvent(new Event('input',{bubbles:true}));},String(v));else await page.locator(s).fill(String(v));await page.locator(s).dispatchEvent('change')};
 ...
 const exportPNG=async name=>{await page.locator('#exportButton').click();const wait=page.waitForEvent('download');await page.locator('#downloadButton').click(); await page.locator('#saveExport').click(); await page.locator('#closeExportPreview').click();const d=await wait;await d.saveAs(path.join(output,name));return sharp(path.join(output,name)).metadata()};
 try{
  ...
  await page.locator('#edgeWidth').click();await page.locator('[data-edge-width="9"]').click();
  await page.locator('#edgeCurve').click();
  await page.locator('#edgeBend').evaluate(el=>{el.value=80;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));});
  await page.locator('#edgeStart').click();await page.locator('#edgeText').click();await page.locator('#sheet .edge-text .rich').fill('一起冒险');
  assert.equal(await page.locator('#sheet .edge-line').getAttribute('stroke-width'),'9');
  ...
  await page.locator('#edgeColor').click();await page.locator('#edgeColor').click();
  ...
  await page.locator('#legendPosition [data-option="bottom-left"]').click();
  ...
  await page.locator('#legendPosition [data-option="top-right"]').click();
