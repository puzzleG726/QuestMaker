...
(async()=>{
  ...
  const download=async name=>{await page.locator('#exportButton').click();const wait=page.waitForEvent('download');await page.locator('#downloadButton').click(); await page.locator('#saveExport').click(); await page.locator('#closeExportPreview').click();const file=await wait;await file.saveAs(path.join(out,name));const stats=await sharp(path.join(out,name)).stats();assert(stats.channels.some(c=>c.stdev>10));return sharp(path.join(out,name)).metadata();};
  try{
    ...
    await page.locator('#sheet [data-knob="voice-0"]').click();await page.locator('#knobShape [data-option="rectangle"]').click();await num('#knobLength',54);
    assert.equal(await page.locator('#sheet [data-knob="voice-0"]').evaluate(e=>e.offsetWidth),54);
    await page.locator('#knobShape [data-option="circle"]').click();assert.equal(await page.locator('#sheet [data-knob="voice-0"]').evaluate(e=>getComputedStyle(e).borderRadius),'50%');
    await page.locator('#knobShape [data-option="heart"]').click();assert.equal(await page.locator('#sheet [data-knob="voice-0"] svg').count(),1);
    ...
    await page.locator('#edgeColor').click();await page.locator('#edgeStart').click();await page.locator('#edgeEnd').click();await page.locator('#edgeWidth').click();await page.locator('[data-edge-width="6"]').click();
    assert.equal(await page.locator('#sheet .edge-line').getAttribute('stroke'),await page.locator('[data-legend-color="code-1"]').inputValue());
    const oldPath=await page.locator('#sheet .edge-line').getAttribute('d');await page.locator('#edgeCurve').click();await num('#edgeBend',-100);await page.keyboard.press('Escape');assert.notEqual(await page.locator('#sheet .edge-line').getAttribute('d'),oldPath);
